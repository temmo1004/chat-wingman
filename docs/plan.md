...
- **Android**：`SYSTEM_ALERT_WINDOW` + 前景 Service + WindowManager，
  Messenger chat head 標準做法，模擬器可 demo → **主產品與主 demo**
- **iOS**：無 overlay API，做不到浮動球；現行跨平台路線為 Back Tap 觸發的
  Apple 捷徑（Take Screenshot → Extract Text from Image → JSON 後端）
...
| 手機端 OCR | Android 用 ML Kit；iOS 捷徑用 Apple Live Text | 低 |
| 回覆面板 | 同一個 overlay window 展開 Compose UI | 低 |
| AI 生成 | OCR 文字 → Flask 後端 → DeepSeek → 固定 JSON（三風格＋解說＋聊死指數） | 低 |
| 複製 | ClipboardManager | 零 |

**資料邊界**：截圖只在手機端用於 OCR，後端只接收
`application/json` 的 `{ "text": "...", "demo": false }`。DeepSeek 金鑰只存在後端。
...
- **殺手應用**：對輸入框節點 `ACTION_SET_TEXT`——demo 從「複製→切回→貼上」
  ...
  （送出仍是用戶手按，不踩自動代發紅線）
- 分層：讀內容走「截圖 + 手機端 OCR」（文字氣泡跨 app 通用）；
  定位與填字走節點樹；節點讀 TextView 可當 OCR 失敗時的文字備援
...
| C | Prompt 工程 + demo 假資料 | DeepSeek 三風格口吻、demo 模式開關（模型失敗可演） |
| D | 簡報 + demo 排練 | 故事線：快被句點 → 軍師救場 → 約到人 |

## 時間表

1. **第 1 hr**：圓球浮起來、可拖曳、點擊有反應（風險最高，最先做）
2. **第 2-3 hr**：截圖 → 手機端 OCR → 文字 API → 面板顯示回覆，全流程打通
...
- DeepSeek 金鑰／API 掛：Android 退回本地假資料；iOS 可將 JSON
  `demo` 改為 `true`（iOS 仍需連得到後端）
